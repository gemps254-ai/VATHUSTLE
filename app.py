...
def create_full_vat_report(s_data, p_data, pin, period, o_v, i_v, n_v):
    ...
    pdf.cell(0, 10, "Computer-generated summary by GEMPS KE.", 0, 0, 'C')
...
with st.sidebar:
    ...
    if kra_pin:
        if not is_valid_pin: st.error("⚠️ Invalid PIN format.")
    ...
    st.download_button("Download Template", generate_excel_template(), "BulkVAT_template.xlsx")
...
if kra_pin and is_valid_pin:
    ...
    with c3: st.metric("Net Position", f"KES {abs(net_payable):,.0f}", 
                      delta="Due to KRA" if net_payable > 0 else "VAT Credit",
                      delta_color="inverse" if net_payable > 0 else "normal")
    st.divider()
else:
    st.info("👋 Welcome! Enter your KRA PIN in the sidebar to start.")
...
with tab1:
    if not kra_pin: st.info("👋 Enter KRA PIN to start.")
        ...
        with st.form("transaction_form", clear_on_submit=True):
            ...
            with col1:
                t_date = st.date_input("Invoice Date", value=st.session_state.scanned_date)
        ...
        try:
            ...
            def proc(df, is_s):
                ...
                for _, r in df.dropna(subset=['Amount']).iterrows():
                    ...
                    res.append({"UserPIN": kra_pin, "Date": str(r.get('Date (YYYY-MM-DD)', today)).split(" ")[0], 
                                "CounterpartyPIN": str(r.get('CounterpartyPIN', '')), "Total": int(round(t)), 
                                "VAT": int(round(v)), "eTIMS": "Yes", "Category": "Sales" if is_s else "Purchases"})
                return pd.DataFrame(res)
            q_df = pd.concat([proc(up_sales, True), proc(up_purch, False)], ignore_index=True)
            if not q_df.empty:
                edited = st.data_editor(q_df, use_container_width=True, hide_index=True)
...
with tab3:
    if not kra_pin: st.info("👋 Enter KRA PIN.")
        ...
        if st.button(f"Generate Report for {sel_m} {sel_y}"):
            ...
            u_s = s_df[(s_df['UserPIN'] == kra_pin) & (s_df['Date'].astype(str).str.startswith(f_str))] if not s_df.empty else pd.DataFrame()
            u_p = p_df[(p_df['UserPIN'] == kra_pin) & (p_df['Date'].astype(str).str.startswith(f_str))] if not p_df.empty else pd.DataFrame()
        ...
        if rd := st.session_state.report_data:
            ...
            b1, b2 = st.columns(2)
            if b1.button("📄 Prepare Final PDF"):
                st.session_state.pdf_report_bytes = create_full_vat_report(rd["u_s"], rd["u_p"], kra_pin, rd["period"], rd["o_v"], rd["i_v"], rd["n_v"])
            
            if pdf_data := st.session_state.pdf_report_bytes:
                b2.download_button("📥 Download PDF", data=pdf_data, file_name=f"VAT_Report_{rd['period']}.pdf", mime="application/pdf")
